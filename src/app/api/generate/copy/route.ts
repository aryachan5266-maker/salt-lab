// 咸聊AI · 文案生成 API · 流式 SSE
// POST /api/generate/copy
// 接收角色+输入+标签 → 拼结构化 prompt → 调大模型 → 流式返回
...
export async function POST(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        sendSSE('meta', { role, input, industry, gender });
        ...
        if (parsed) {
          sendSSE('done', { ok: true, data: parsed, source: 'llm' });
        } else {
          // 流式输出了文本但未拿到结构化JSON，用完整文本构造一个基础结构
          sendSSE('done', {
            ok: true,
            data: {
              titles: [input.slice(0, 20) + '...'],
              body: fullContent,
              tags: ['#咸聊AI', `#${industry || '营销'}`, '#内容创作'],
              platform: '小红书',
