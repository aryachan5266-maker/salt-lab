...
export default function AnalyticsPage() {
  ...
  const kpis = [
    { label: '累计曝光', value: Math.round(data.totalReach * periodMultiplier), format: (v: number) => `${(v / 10000).toFixed(1)}w`, delta: '+12.4%' },
    { label: '累计互动', value: Math.round(data.totalEngagement * periodMultiplier), format: (v: number) => `${(v / 1000).toFixed(1)}k`, delta: '+8.6%' },
    { label: '爆款率', value: data.hitRate, format: (v: number) => `${v}%`, delta: '+2.1%' },
    { label: '平均互动率', value: data.avgEngagement, format: (v: number) => `${v}%`, delta: '+0.3%' },
    { label: '粉丝净增', value: Math.round(data.fansNet * periodMultiplier), format: (v: number) => v.toLocaleString(), delta: '+18%' },
    { label: 'ROI', value: data.roi, format: (v: number) => `×${v}`, delta: '+0.4' },
  ];
  ...
  const areaD = pathD + ` L${points[points.length - 1][0].toFixed(1)},${H - P} L${points[0][0].toFixed(1)},${H - P} Z`;
          ...
          <>
            <div className="flex rounded-md border border-zinc-800 bg-zinc-900 p-0.5">
              {([['48h', '48小时'], ['7d', '7天'], ['30d', '30天']] as const).map(([k, l]) => (
                <button key={k} onClick={() => setPeriod(k)} className={`rounded px-3 py-1 text-sm ${period === k ? 'bg-rose-500/15 text-rose-300' : 'text-zinc-400'}`}>{l}</button>
              ))}
            </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
        {kpis.map((k, i) => (
          <Card key={i}>
            ...
            <div className="kpi-num mt-1 text-2xl font-semibold text-rose-100">{k.format(k.value)}</div>
          ...
          <Card>
            ...
            <CardContent>
              <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
                ...
                <text x={P} y={H - 4} fill="currentColor" fontSize="10" opacity="0.4">11/24</text>
                <text x={W - P} y={H - 4} fill="currentColor" fontSize="10" opacity="0.4" textAnchor="end">12/23</text>
              </svg>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="space-y-2">
                {data.topPerformers.map((t, i) => (
                  <button key={t.id} onClick={() => setDetailItem({ ...t, type: 'top' })} className="flex w-full items-center gap-3 rounded-md border border-zinc-800 bg-zinc-900/40 p-2 text-left transition hover:border-rose-500/30">
            ...
            <CardContent>
              <div className="space-y-2">
                {data.bottomPerformers.map((t, i) => (
                  <button key={t.id} onClick={() => setDetailItem({ ...t, type: 'bottom' })} className="flex w-full items-center gap-3 rounded-md border border-zinc-800 bg-zinc-900/40 p-2 text-left transition hover:border-zinc-700">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2 text-xs">
                  {([['engagement', '互动率'], ['reach', '曝光'], ['time', '时间']] as const).map(([k, l]) => (
                    <button key={k} onClick={() => setSortBy(k)} className={`rounded px-2 py-0.5 ${sortBy === k ? 'bg-rose-500/15 text-rose-300' : 'text-zinc-400'}`}>{l}</button>
                  ))}
                </div>
              </div>
            </CardHeader>
                  ...
                  return (
                    <div key={s.id} className={`rounded-md border p-3 ${adopted ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-rose-500/20 bg-rose-500/5'}`}>
                      <div className="mb-1 flex items-center justify-between text-xs">
                        ...
                        <span className="text-zinc-500">置信 {Math.round(s.confidence * 100)}%</span>
                      </div>
      ...
      {detailItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setDetailItem(null)}>
          <div className="w-[480px] max-w-[90vw] rounded-xl border border-zinc-800 bg-zinc-950 p-6" onClick={(e) => e.stopPropagation()}>
            ...
            {detailItem.coverUrl && <img src={detailItem.coverUrl} className="mb-3 aspect-[3/4] w-full rounded-md object-cover" alt="" />}
            <div className="space-y-2 text-sm text-zinc-300">
              ...
              {detailItem.reason && <div className="text-rose-300">归因：{detailItem.reason}</div>}
            </div>
          </div>
        </div>
      )}
